...
class Grafo:
    ...
    def matrizDistancias(self):
        ...
        #Cria uma matrix (lista de listas) com valor inicial infinito para tratar caminhos minimos
        matrizDist = [math.inf] * self.qtdVertices()
        for i in range(self.qtdVertices()):
            matrizDist [i] = [math.inf] * self.qtdVertices()

        # Seta a distancia de si mesmo para zero
        for i in range(self.qtdVertices()):
            for j in range(self.qtdVertices()):
                if i == j:
                    matrizDist[i][j] = 0
        
        #Percorre a matriz e salvou seu peso on tem arestas entre os vertices
        for i in range(self.qtdVertices()):
            for j in range(self.qtdVertices()):
                if self.haAresta(i,j):
                    matrizDist[i][j] = self.peso(i,j)

        #Obtem uma matriz de caminhos
        return matrizDist